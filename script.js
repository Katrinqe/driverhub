...
let navWatchId = null; // Für das Folgen im Navi-Modus
...
window.addEventListener('load', () => {
    ...
    if(recenterBtn) {
        // ... (Old Logic removed for brevity here, is in HTML onclick)
    }
...
function initNavMap() {
    ...
    setTimeout(() => {
        // Falls schon eine Navi-Karte da ist, zerstören (für frischen Start)
        if(navMap) {
             navMap.remove();
             navMap = null;
        }
        ...
        if(navigator.geolocation) {
            navigator.geolocation.getCurrentPosition(pos => {
                ...
                routingControl.on('routesfound', function(e) {
                     document.getElementById('btn-start-guidance').style.display = 'block';
                });
                ...
                // Fallback ohne GPS (Standard Routing Control)
                routingControl = L.Routing.control({
                    waypoints: [ null, null ],
...
function startNaviFollow() {
    // Hier können wir später die Logik zum Verfolgen einbauen
    // Vorerst: Zoomt rein auf den User (Startpunkt)
    if(navMap && routingControl) {
        ...
        if (navigator.geolocation) { 
            ...
            navWatchId = navigator.geolocation.watchPosition(pos => {
                const lat = pos.coords.latitude;
                const lng = pos.coords.longitude;
                // Navi Marker aktualisieren (wenn wir einen hätten) oder Map bewegen
                navMap.setView([lat, lng], 18);
