...
window.switchSocialTab = function(tabName) { 
    ...
    // 2. ALLE Bereiche verstecken (Wichtig für Fix!)
...
function initUserProfile() { 
    db_fire.collection('users').doc(currentUser.uid).onSnapshot(doc => { 
        if (!doc.exists) { db_fire.collection('users').doc(currentUser.uid).set({ email: currentUser.email, username: currentUser.email.split('@')[0], bio: "DriverHub User", photoURL: "", followers: 0, following: 0, searchKey: currentUser.email.split('@')[0].toLowerCase(), joined: new Date() }, {merge:true}); } 
        else { const data = doc.data(); if(data.username) { currentUserName = data.username; updateTimeGreeting(); } } 
    }); 
}
...
function loadChatInbox() { 
    ...
    db_fire.collection('chats').where('participants', 'array-contains', currentUser.uid).onSnapshot(snap => { 
        list.innerHTML = ""; if(snap.empty) { list.innerHTML = "<p style='color:#666;text-align:center;'>Keine Nachrichten.</p>"; return; } 
        snap.forEach(doc => { 
...
function loadFriendsList() { 
    ...
    db_fire.collection('follows').where('followerId', '==', currentUser.uid).get().then(snap => { 
        list.innerHTML = ""; if(snap.empty) { list.innerHTML = "<p style='color:#666;text-align:center;'>Du folgst niemandem.</p>"; return; } 
        snap.forEach(fDoc => { 
...
function loadFriendsFeed() { 
    ...
    db_fire.collection('follows').where('followerId', '==', currentUser.uid).get().then(snap => { 
        const followingIds = []; snap.forEach(doc => followingIds.push(doc.data().followingId)); followingIds.push(currentUser.uid); 
        feedUnsubscribe = db_fire.collection('posts').orderBy('timestamp', 'desc').limit(50).onSnapshot(postSnap => { 
            const container = app.comm.feed; const msg = document.getElementById('no-friends-msg'); container.innerHTML = ""; let hasPosts = false; 
            postSnap.forEach(doc => { const post = doc.data(); if(followingIds.includes(post.uid)) { renderPost(post, container, doc.id); hasPosts = true; } }); 
...
function renderPost(post, container, postId) { 
    ...
    let avaStyle = `background: var(--accent-blue);`; let avaContent = post.author.charAt(0).toUpperCase(); if(post.authorPic) { avaStyle = `background-image: url('${post.authorPic}');`; avaContent = ""; } 
    const likes = post.likes || []; const isLiked = likes.includes(currentUser.uid); 
    let deleteBtn = ""; if(post.uid === currentUser.uid) { deleteBtn = `<button class="btn-delete-post" style="float:right; background:none; border:none; color:#666;" onclick="deletePost('${postId}')"><i class="fa-solid fa-trash"></i></button>`; } 
    const div = document.createElement('div'); div.className = 'post-card'; 
    div.innerHTML = `<div class="post-header" onclick="openProfile('${post.uid}')"><div class="post-avatar" style="${avaStyle}">${avaContent}</div><span class="post-user">${escapeHtml(post.author)}</span><span class="post-date">${date}</span></div>${deleteBtn}<div class="post-content">${escapeHtml(post.text)}</div><div class="post-actions"><span style="color:${isLiked?'#ff3b30':'#888'}; cursor:pointer;" onclick="toggleLike('${postId}', ${isLiked})"><i class="${isLiked?'fa-solid':'fa-regular'} fa-heart"></i> ${likes.length}</span></div>`; 
...
document.getElementById('btn-back-feed').addEventListener('click', () => { 
    app.comm.profileView.style.display = 'none'; app.comm.mainView.style.display = 'block'; 
});

function openProfile(uid) { 
    viewingUserUid = uid; const isMe = (currentUser.uid === uid); 
    // WICHTIG: Alles verstecken, dann Profil zeigen
    ...
    db_fire.collection('users').doc(uid).onSnapshot(doc => { if(!doc.exists) return; const data = doc.data(); pName.innerText = data.username; pBio.innerText = data.bio || "Keine Bio."; if(data.photoURL) pImg.style.backgroundImage = `url('${data.photoURL}')`; document.getElementById('p-followers').innerText = data.followers || 0; document.getElementById('p-following').innerText = data.following || 0; }); 
    ...
    if(isMe) { document.getElementById('btn-edit-img').style.display = 'flex'; document.getElementById('btn-edit-bio').style.display = 'inline-block'; document.getElementById('btn-follow-action').style.display = 'none'; document.getElementById('btn-msg-action').style.display = 'none'; } 
    else { document.getElementById('btn-edit-img').style.display = 'none'; document.getElementById('btn-edit-bio').style.display = 'none'; document.getElementById('btn-follow-action').style.display = 'inline-block'; document.getElementById('btn-msg-action').style.display = 'inline-block'; checkIfFollowing(uid); } 
}
...
function loadMessages() { 
    ...
    db_fire.collection('chats').doc(activeChatId).collection('messages').orderBy('timestamp').onSnapshot(snap => { 
        msgBox.innerHTML = ""; snap.forEach(doc => { const m = doc.data(); const bubble = document.createElement('div'); bubble.className = `chat-bubble ${m.senderId === currentUser.uid ? 'me' : 'them'}`; bubble.innerText = m.text; msgBox.appendChild(bubble); }); 
